...
def _check_top_level_only(ctx: AnalysisContext):
    for all_paths in ctx.attrs.top_level_only_paths:
        all_paths = list(all_paths)
        target = all_paths.pop()
        remainder = filter(lambda t: not str(t.label).startswith("fbcode//bz/app/bz:"), all_paths)
...
_CLIENT_BIN = "fbcode//bz/app/bz:bz_client-bin"

_BUCK2_BIN = "fbcode//bz/app/bz:bz-bin"
