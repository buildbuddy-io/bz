...
PYTHON_TEST_EXAMPLE_TARGET = "//tests/targets/rules/python/coverage:test"
...
async def test_python_coverage(buck: Buck) -> None:
    ...
    assert "workspace/buck2/tests/targets/rules/python/coverage/lib.py" in paths, str(
        paths
    )
...
async def test_python_coverage_filtering_by_folder(buck: Buck) -> None:
    folder_to_collect = "workspace/buck2/tests/targets/rules/python/coverage"
    with tempfile.NamedTemporaryFile("w") as covfile:
        await buck.test(
            "@root//mode/dbgo-cov",
            PYTHON_TEST_EXAMPLE_TARGET,
...
async def test_python_coverage_filtering_by_file(buck: Buck, tmp_path: Path) -> None:
    file_to_collect_coverage = "workspace/buck2/tests/targets/rules/python/coverage/lib.py"
...
async def test_python_coverage_filtering_by_file_with_base_module_remap(
    ...
    result = await collect_coverage_for(
        buck,
        tmp_path,
        target="//tests/targets/rules/python/coverage:test_using_base_module_remapped_lib",
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
    )
...
async def test_python_standalone_xar_coverage_filtering_by_file(
    ...
    file_to_collect_coverage = (
        "workspace/testinfra/playground/python/simple/simple_test.py"
    )
    result = await collect_coverage_for(
        buck,
        tmp_path,
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
        target="root//testinfra/playground/python/simple:simple_standalone_xar_test",
        mode="@root//mode/opt",
    )
...
async def test_python_standalone_zip_coverage_filtering_by_file(
    ...
    result = await collect_coverage_for(
        ...
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
        target="root//testinfra/playground/python/simple:simple_standalone_test",
        mode="@root//mode/opt",
    )
...
async def test_python_coverage_filtering_by_file_on_cinder_target(
    ...
    file_to_collect_coverage = (
        "workspace/buck2/tests/targets/rules/python/coverage/test.py"
    )
    result = await collect_coverage_for(
        buck,
        tmp_path,
        target="//tests/targets/rules/python/coverage:simple_cinder_unittest",
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
    )
...
async def test_python_coverage_filtering_by_source_file_on_cpp_dep(
    ...
    file_to_collect_coverage = (
        "workspace/buck2/tests/targets/rules/python/coverage/cpp_lib.cpp"
    )
    result = await collect_coverage_for(
        buck,
        tmp_path,
        target="//tests/targets/rules/python/coverage:test_with_cpp_dep",
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
    )
...
async def test_python_coverage_filtering_by_header_file_on_cpp_dep(
    ...
    header_file = "workspace/buck2/tests/targets/rules/python/coverage/cpp_lib.h"
...
async def test_python_coverage_filtering_by_file_on_ligen_cpp_dep(
    ...
    file_to_collect_coverage = (
        "workspace/buck2/tests/targets/rules/python/coverage/adder_ligen.cpp"
    )
    result = await collect_coverage_for(
        buck,
        tmp_path,
        target="//tests/targets/rules/python/coverage:test_with_ligen_cpp_dep",
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
    )
...
async def test_python_selective_coverage_in_arvr(buck: Buck, tmp_path: Path) -> None:
    ...
    result = await collect_coverage_for(
        buck,
        tmp_path,
        target="root//arvr/projects/tcc_playground/python_unittest:test_example",
        folder_filter=[],
        file_filter=[file_to_collect_coverage],
        mode="@root//arvr/mode/platform010/opt",
    )
