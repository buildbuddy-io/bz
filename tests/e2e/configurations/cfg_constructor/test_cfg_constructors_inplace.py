...
async def test_cfg_constructor_without_modifiers_returns_same_configuration(
    ...
    result = await buck.cquery(
        "//tests/e2e/configurations/cfg_constructor/test_clear_package_modifiers_data/test_cfg_constructor_data:no_modifiers",
        "-A",
    )
...
async def test_cfg_constructor_with_target_modifiers(buck: Buck) -> None:
    result = await buck.cquery(
        "//tests/e2e/configurations/cfg_constructor/test_clear_package_modifiers_data/test_cfg_constructor_data:has_target_modifier",
        "-A",
    )
    result = json.loads(result.stdout)
    assert len(result) == 1
...
async def test_invoke_cfg_constructors_with_cli_modifier_validation(buck: Buck) -> None:
    ...
    await expect_failure(
        buck.cquery(
            "//tests/e2e/configurations/cfg_constructor/test_clear_package_modifiers_data/test_cfg_constructor_data:has_target_modifier",
            "--modifier=//tests/e2e/configurations/cfg_constructor/test_clear_package_modifiers_data/test_cfg_constructor_data:some_constraint_value",
        ),
