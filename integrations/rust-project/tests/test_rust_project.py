...
async def test_workspaces(buck: Buck) -> None:
    result_raw = await buck.bxl(
        ...
        "--",
        "--targets",
        "//bz/integrations/rust-project/tests/targets/foo:e",
    )
    result: Dict[str, Any] = json.load(open(result_raw.stdout.rstrip()))
    assert result["expanded_targets"] == [
        "fbcode//bz/integrations/rust-project/tests/targets/bar:d",
        "fbcode//bz/integrations/rust-project/tests/targets/foo:a",
    ]
    target_and_in_workspace = {
        t: v["in_workspace"] for t, v in result["resolved_deps"].items()
    }
    expected_subset = {
        "fbcode//bz/integrations/rust-project/tests/targets/foo:a": True,
        "fbcode//bz/integrations/rust-project/tests/targets/bar:c": False,
        "fbcode//bz/integrations/rust-project/tests/targets/bar:d": True,
        "fbcode//bz/integrations/rust-project/tests/targets/foo:e": True,
        "fbcode//bz/integrations/rust-project/tests/targets/foo:f": True,
    }
    ...
    result_raw = await buck.bxl(
        ...
        "--",
        "--targets",
        "//bz/integrations/rust-project/tests/targets/bar:c",
    )
    result: Dict[str, Any] = json.load(open(result_raw.stdout.rstrip()))
    assert result["expanded_targets"] == [
        "fbcode//bz/integrations/rust-project/tests/targets/bar:c"
    ]
    target_and_in_workspace = {
        t: v["in_workspace"] for t, v in result["resolved_deps"].items()
    }
    ...
    expected_subset = {
        "fbcode//bz/integrations/rust-project/tests/targets/bar:c": True,
        "fbcode//bz/integrations/rust-project/tests/targets/foo:e": False,
        "fbcode//bz/integrations/rust-project/tests/targets/foo:f": False,
    }
...
async def test_alias(buck: Buck) -> None:
    result_raw = await buck.bxl(
        ...
        "--",
        "--targets",
        "fbcode//bz/integrations/rust-project/tests/targets/alias/...",
    )
    result: Dict[str, Any] = json.load(open(result_raw.stdout.rstrip()))
    assert result["expanded_targets"] == [
        "fbcode//bz/integrations/rust-project/tests/targets/alias:l",
        "fbcode//bz/integrations/rust-project/tests/targets/alias:l_alias",
    ]
...
async def test_resolve_owning_buildfile_no_extra_targets(buck: Buck) -> None:
    ...
    owners.sort()
    assert owners == [
        "fbcode//bz/integrations/rust-project/tests/targets/foo:f",
        "fbcode//bz/integrations/rust-project/tests/targets/foo:f-unittest",
    ]
...
async def test_exclude_workspaces(buck: Buck) -> None:
    ...
    result: Dict[str, Any] = json.load(open(result_raw.stdout.rstrip()))
    assert result["expanded_targets"] == [
        "fbcode//bz/integrations/rust-project/tests/targets/foo:e",
    ]
...
async def test_fallback_compatible_with(buck: Buck) -> None:
    result_raw = await buck.bxl(
        ...
        "--",
        "--targets",
        "//bz/integrations/rust-project/tests/targets/foo:g_with_compatibility",
    )
    result: Dict[str, Any] = json.load(open(result_raw.stdout.rstrip()))
