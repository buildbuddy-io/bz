...
`<FBSOURCE>/fbcode/scripts/$USER/bz_lab`.
...
Navigate to the `bz_lab` folder we created in the
...
...
BUILD SUCCEEDED
fbcode//scripts/$USER/bz_lab/greeter_lib:library /.../greeter_lib/__library__/out/LPPMD/liblibrary-1527a50c.rmeta
```
...
...
BUILD SUCCEEDED
root//bz_lab/greeter_lib:library /.../greeter_lib/__library__/out/LPPMD/liblibrary-1527a50c.rmeta
```
...
In the `greeter_bin` directory (i.e., `bz_lab/greeter_bin/BUCK`), update the
BUCK file.
...
rust_binary(
    ...
    deps = ["fbcode//scripts/$USER/bz_lab/greeter_lib:library"],
)
```
...
rust_binary(
    ...
    srcs = ["src/main.rs"],
    # Add the dep to our library
    deps = ["root//bz_lab/greeter_lib:library"],
)
```
...
<code>fbcode//scripts/$USER/bz_lab/greeter_lib:library</code> :
<code>root//bz_lab/greeter_lib:library</code>;
...
- { isInternal() ? <code>deps =
  ["fbcode//scripts/$USER/bz_lab/greeter_lib:library"]</code> :
  <code>deps = ["root//bz_lab/greeter_lib:library"]</code> }: This is the
  crucial new part!
...
```bash
buck2 run root//bz_lab/greeter_bin:main
```
...
`fbcode//bz/docs/bz_lab/logging_lib:logging_lib`. </FbInternalOnly>
<OssOnly> You can find it at
https://github.com/facebook/buck2/tree/main/docs/bz_lab/logging_lib and copy
the folder into `bz_lab` folder. </OssOnly>

Our first step is to make our existing greeter_lib use this new logging_lib.

1. Update `greeter_lib/BUCK`:

Now, modify `bz_lab/greeter_lib/BUCK` to declare a dependency on
`logging_lib`.
...
rust_library(
    ...
    deps = [
        ...
        "fbcode//bz/docs/bz_lab/logging_lib:logging_lib",
    ],
)
```
...
rust_library(
    ...
    deps = [
        # Add the dep to our logging_lib
        "root//bz_lab/logging_lib:logging_lib",
    ],
)
```
...
error[E0433]: failed to resolve: use of unresolved module or unlinked crate `logging_lib`
 --> fbcode/scripts/$USER/bz_lab/greeter_bin/src/main.rs:4:5
  |
...
error[E0433]: failed to resolve: use of unresolved module or unlinked crate `logging_lib`
 --> fbcode/scripts/$USER/bz_lab/greeter_bin/src/main.rs:9:5
  |
...
rust_binary(
    ...
    srcs = ["src/main.rs"],
    deps = [
        "fbcode//scripts/$USER/bz_lab/greeter_lib:library",
        "fbcode//bz/docs/bz_lab/logging_lib:logging_lib",  # Add this line
    ],
)
```
...
rust_binary(
    ...
    srcs = ["src/main.rs"],
    deps = [
        "root//bz_lab/greeter_lib:library",
        "root//bz_lab/logging_lib:logging_lib",  # Add this line
    ],
)
```
