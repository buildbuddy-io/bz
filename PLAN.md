...
- Bzlmod module-extension `native.register_toolchains(...)` calls no longer feed global registered toolchains. This matches Bazel's `RegisteredToolchainsFunction`, which reads `register_toolchains(...)` declarations from selected `MODULE.bazel` modules rather than module-extension evaluation output.
- Bazel toolchain resolution now filters registered `toolchain(...)` targets with configured `target_settings`, `target_compatible_with`, `exec_compatible_with`, and `use_target_platform_constraints` before reading the lazy `toolchain` implementation label. This keeps toolchain implementation repos lazy and matches Bazel's `ConfigMatchingUtil`/`SingleToolchainResolutionFunction` selection order.
- Initial `~/Code/bazel` `src:bazel` validation with those toolchain changes avoids the previous broad cross-platform generated-toolchain expansion, but still exceeded the disk guard while loading `bazel_tools//tools/launcher`. The remaining blocker is broad external repository materialization in the Bazel source build path, including network-sensitive deps such as `libpfm-4.11.0`, not a completed `src:bazel` build yet.
