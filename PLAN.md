...
After wiring generic repository-rule execution, the smoke loads the root target and downloaded `rules_go`, executes the real `go_sdk` extension, materializes `@io_bazel_rules_nogo`, materializes `@go_toolchains`, resolves the registered `@go_toolchains//:all` pattern through the `rules_go` module repo mapping, and resolves the statically named extension-internal `@go_default_sdk` repo. It now stops at the next architectural boundary:
...
- A Bazel-valid rules_go smoke repo with direct `rules_proto` visibility now resolves root and downloaded-module aliases separately, loads rules_go through its real downloaded module cell, resolves `@io_bazel_rules_nogo` and `@go_toolchains` to generated cells from the real `go_sdk` extension, invokes the real downloaded extension, executes the recorded repository-rule implementations for those generated repos, and stops when `@go_toolchains` references dynamically emitted same-extension SDK download repos that were not present in the pre-analysis cell graph.
