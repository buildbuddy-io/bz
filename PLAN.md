...
- Bazel Starlark test rules declared with `rule(test = True, ...)` now receive Bazel's implicit test attrs (`size`, `timeout`, `flaky`, `shard_count`, `local`, and `args`).
...
After wiring generic repository-rule execution and repository_ctx download/extract support, the smoke loads the root target and downloaded `rules_go`, executes the real `go_sdk` extension, materializes `@io_bazel_rules_nogo`, materializes `@go_toolchains`, resolves the registered `@go_toolchains//:all` pattern through the `rules_go` module repo mapping, and materializes the real `@go_default_sdk` SDK repository. A direct build of `bzlmod_rules_go_0_57_0_go_sdk_go_default_sdk//:files` now succeeds with `--target-platforms=platforms//host:host`. The root smoke now stops at the next architectural boundary:

```text
unknown cell alias: `rules_go__download_0_darwin_amd64`
```

That repo name is produced dynamically by the downloaded `go_sdk` module extension implementation while emitting repository-rule calls. The hard cutover requires evaluating module extensions before finalizing the bzlmod cell graph, so every emitted generated repo becomes a real external cell and generated repos get the complete same-extension internal repo mapping. We should not reintroduce Rust-side special cases for those names.
...
- Evaluate module extensions during bzlmod resolution, before the cell graph is frozen, and use the emitted repository-rule calls as the source of truth for all generated repo cells and same-extension repo mappings.
...
- Move real module-extension evaluation from generated-repo materialization into bzlmod resolution so dynamically emitted repos like `rules_go__download_0_darwin_amd64` are real generated cells before analysis starts.
- Use the emitted repository-rule call list to build complete same-extension internal repo mappings for generated repos.
- Retire the remaining module-specific generated-repo materializers as their extensions become executable.

Current validation boundary:

- A Bazel-valid rules_go smoke repo with direct `rules_proto` visibility now resolves root and downloaded-module aliases separately, loads rules_go through its real downloaded module cell, resolves `@io_bazel_rules_nogo` and `@go_toolchains` to generated cells from the real `go_sdk` extension, invokes the real downloaded extension, executes the recorded repository-rule implementations for those generated repos, downloads and extracts the real Go SDK archive for `@go_default_sdk`, and stops when `@go_toolchains` references dynamically emitted same-extension SDK download repos that were not present in the pre-analysis cell graph.
