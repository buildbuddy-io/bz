...
use bz_analysis::analysis::calculation::profile_analysis;
use bz_cli_proto::TargetCfg;
use bz_cli_proto::profile_request::ProfileOpts;
use bz_cli_proto::target_profile::Action;
...
) -> bz_error::Result<Arc<StarlarkProfileDataAndStats>> {
...
async fn generate_profile(
    ...
    match action {
        ...
        Action::Loading => {
            ...
            let profiles = bz_util::future::try_join_all(resolved.specs.into_iter().map(
