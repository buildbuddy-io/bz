...
fn test_source_location_no_attrs() {
    ...
    assert!(
        e.source_location()
            .to_string()
            .starts_with("bz_error/src/derive_tests.rs::NoAttrsStruct::")
    );
    let e: crate::Error = NoAttrsEnum::Variant.into();
    assert!(
        e.source_location()
            .to_string()
            .starts_with("bz_error/src/derive_tests.rs::NoAttrsEnum::Variant::")
    );
}
...
fn test_enum_with_type_option() {
    ...
    assert!(
        e.source_location()
            .to_string()
            .starts_with("bz_error/src/derive_tests.rs::EnumWithTypeOption::Variant::")
    );
}
