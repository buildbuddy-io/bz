...
/// A trait that exposes only the bz_data-dependent bits of an EventDispatcher.
///
/// This is so we can use a `LateBinding` and dispatch events from bz_core,
/// which can't take a depndency on bz_event (where `EventDispatcher` is defined).
