...
pub trait EventSubscriber: Send {
    ...
    /// stdout.
    async fn handle_output(&mut self, _raw_output: &[u8]) -> bz_error::Result<()> {
        Ok(())
    }
    ...
    async fn handle_events(&mut self, _event: &[Arc<BuckEvent>]) -> bz_error::Result<()> {
        Ok(())
    }
    async fn handle_command_result(
        &mut self,
        _result: &bz_cli_proto::CommandResult,
    ) -> bz_error::Result<()> {
        Ok(())
    }
    ...
    fn handle_daemon_started(&mut self, _reason: bz_data::DaemonWasStartedReason) {}
    ...
    async fn finalize(self: Box<Self>) -> bz_error::Result<()> {
        Ok(())
    }
}
