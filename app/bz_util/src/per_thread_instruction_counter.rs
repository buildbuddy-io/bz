...
impl PerThreadInstructionCounter {
    ...
    pub fn init() -> bz_error::Result<Option<PerThreadInstructionCounter>> {
        Self::init_impl()
    }

    #[cfg(target_os = "linux")]
    fn init_impl() -> bz_error::Result<Option<PerThreadInstructionCounter>> {
    ...
    pub fn collect(self) -> bz_error::Result<u64> {
        self.collect_impl()
    }

    #[cfg(target_os = "linux")]
    fn collect_impl(mut self) -> bz_error::Result<u64> {
    ...
    #[cfg(not(target_os = "linux"))]
    fn collect_impl(self) -> bz_error::Result<u64> {
