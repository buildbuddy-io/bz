...
def _get_artifacts_with_owners(files: typing.Any) -> dict[Artifact, dict[str, ConfiguredProvidersLabel]]:
    if type(files) == "dict":
...
def _get_artifact_owner(file: Artifact) -> dict[str, ConfiguredProvidersLabel]:
    if file.owner:
